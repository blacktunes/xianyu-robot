...
import { getSetu, recoed, saveSetu, setuTotal, setuWatchNum, viewed } from "./mysql"
import { socketSetu } from "./socket"
import schedule = require('node-schedule')
import { downloadCache } from './download'

let restrictedList = {}

export function clearRestrictedList() {
 ...
 */
export async function setu(bot: Bot, from: number, fromQQ: number, num: number, fromType: 0 | 1 | 2, tag?: string) {
  const insertId = bot.CQ.getDebug ? 0 : await recoed(bot, fromQQ, from, fromType, num)
  if (fromType === 1 && fromQQ != bot.adminData.qq) {
    if (restrictedList[fromQQ]) {
      restrictedList[fromQQ] += 1
    } else {
      restrictedList[fromQQ] = 1
    }
    if (restrictedList[fromQQ] > bot.config.setu.limit) {
      if (restrictedList[fromQQ] > (bot.config.setu.limit + 3)) {
  ...
  if (bot.config.setu.default === 1) {
    getSetu(bot, num, tag)
      .then(res => {
        const data = formatData(bot, res, 1)
        if (res.length > 0) {
          ...
          bot.send(fromType, from, data.img)
            .then(code => {
              if (code == -1 || code == -1100) {
                ...
                if (!bot.CQ.getDebug) viewed(data.title, data.sql, insertId, 1, res.length)
              }
            })
        } else {
    ...
    axios.get(`${bot.config.setu.api}?${bot.config.setu.apikey ? 'apikey=' + bot.config.setu.apikey : ''}${tag ? '&keyword=' + tag : ''}&size1200=1&num=${num}`)
      .then((res) => {
        if (res.data.code === 0) {
          const data = formatData(bot, res.data.data, 0)
          saveSetu(data.saveSql)
          ...
          bot.send(fromType, from, data.url)
            .then(code => {
              if (code == -1 || code == -1100) {
                ...
                if (!bot.CQ.getDebug) viewed(data.title, data.sql, insertId, 0, res.data.data.length)
              }
            })
        } else {
          handleErr(bot, res.data.code, res.data.quota_min_ttl, from, fromQQ, fromType)
        }
      })
      .catch(err => {
 ...
 */
function handleErr(bot: Bot, code: number, quotaMinTtl: number, from: number, fromQQ: number, fromType: 0 | 1 | 2) {
  if (code === 401) {
 ...
 */
function formatData(bot: Bot, data: Array<any>, type: 0 | 1) {
  if (type === 0) {
...
export function startCopy(bot: Bot, fromQQ?: number) {
  if (!copying) {
    printTime(`第${times}次请求···`, 10)
    axios.get(`${bot.config.setu.api}?${bot.config.setu.apikey ? 'apikey=' + bot.config.setu.apikey : ''}&size1200=1&num=10&r18=2`, {
      timeout: 1000 * 15
    })
      .then(async res => {
        printTime(`第${times}次请求成功 剩余次数：${res.data.quota}`, 20)
        if (res.data.code === 0) {
          await formatAndSave(res.data.data)
 ...
 * @param {number} fromQQ
 */
async function copySetu(bot: Bot, fromQQ?: number) {
  if (bot.config.setu.copy_times) {
    copyEnd(bot, 0, fromQQ)
    return
  }
  if (errTimes >= 5) {
    copyEnd(bot, 2, fromQQ)
    return
  }
  if (!copying) {
    copyEnd(bot, 4, fromQQ)
    return
  }
  if (timeout) {
    copyEnd(bot, 3, fromQQ)
    return
  }
  await bot.sleep(2)
  ++times
 ...
 * @param fromQQ
 */
async function copyEnd(bot: Bot, type: 0 | 1 | 2 | 3 | 4, fromQQ?: number) {
  copying = false
  schedule.cancelJob('copy')
  const endTime = new Date()
  const setuNum = await setuTotal()
  const newNum = initialNum && setuNum ? `新增图量：${setuNum[0] - initialNum}\n` : ''
  const allNum = setuNum ? `总图量：${setuNum[0]}\n未浏览：${setuNum[1]}\n` : ''
  switch (type) {
    case 0:
      bot.send(bot.adminData.type, bot.adminData.type === 0 ? bot.adminData.qq : bot.adminData.id, `${fromQQ && bot.adminData.type !== 0 ? bot.CQCode.at(fromQQ) : ''}搬完了\n${newNum}${allNum}调用次数：${times}\n用时：${(endTime.getTime() - startTime.getTime()) / 1000}s`)
      break
    case 1:
...
async function formatAndSave(data: any) {
...
export function setuStar(bot: Bot, fromType: 1 | 2, id: number) {
  setuWatchNum(id)
    .then(data => {
      if (data.length < 1) {
        if (fromType === 2) {
