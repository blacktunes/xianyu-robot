...
export class CoolQ {
     ...
     * @memberof CoolQ
     */
    constructor(appOption: CQOption, debug: boolean = false) {
    ...
    */
    setDebug(debug: boolean) {
        this.debug = debug
        if (this.debug) {
            printTime(`[setDebug] 应用${this.APP_ID ? ' ' + this.APP_ID + ' ' : ''}已开启debug模式，所有api调用都不会真正执行`, CQLog.LOG_INFO_SUCCESS)
        }
        // else {
            ...
            if (this.appOption.auth.includes(auth) || auth === 1) { // 判断是否有权限执行该操作
                if (bot.isReady()) {
                    ...
                    if (opName.startsWith('send_')) {
                        ...
                        printTime(`执行函数${opName}，执行结果: 成功`, CQLog.LOG_INFO_SUCCESS)
                        return result
                    } else {
                        // printTime(`执行函数 ${opName} , 参数:${JSON.stringify(param)} , 执行结果:${JSON.stringify(result)}`, CQLog.LOG_INFO_SUCCESS)
                        printTime(`执行函数${opName}，执行结果: 成功`, CQLog.LOG_INFO_SUCCESS)
