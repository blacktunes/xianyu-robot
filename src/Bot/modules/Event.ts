      ...
      this
        ...
        .on('notice.notify', (event) => {
          ...
          if (event.sub_type === 'poke') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.user_id.toString())}) 戳了戳 (${colors.white(event.target_id.toString())})`, 'EVENT')
          } else if (event.sub_type === 'lucky_king') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.target_id.toString())}) 获得了 (${colors.white(event.user_id.toString())}) 发出的群红包运气王`, 'EVENT')
            ...
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.user_id.toString())}) 获得了 ${honor}`, 'EVENT')
          }
        })
        .on('notice.group_admin', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          if (event.sub_type === 'set') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.user_id.toString())}) 成为了管理员`, 'EVENT')
          } else if (event.sub_type === 'unset') {
            PrintLog.logWarning(`群${colors.magenta(this.groupList[event.group_id] || '')}(${colors.magenta(event.group_id.toString())}) - (${colors.magenta(event.user_id.toString())}) 被取消了管理员`, 'EVENT')
          }
        })
        .on('notice.group_decrease', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          if (event.sub_type === 'kick') {
            PrintLog.logWarning(`群${colors.magenta(this.groupList[event.group_id] || '')}(${colors.magenta(event.group_id.toString())}) - (${colors.magenta(event.user_id.toString())}) 被 (${colors.magenta(event.operator_id.toString())}) 踢出群聊`, 'EVENT')
          } else if (event.sub_type === 'leave') {
            PrintLog.logWarning(`群${colors.magenta(this.groupList[event.group_id] || '')}(${colors.magenta(event.group_id.toString())}) - (${colors.magenta(event.user_id.toString())}) 退出了群聊`, 'EVENT')
          } else if (event.sub_type === 'kick_me') {
            PrintLog.logWarning(`${this.Bot.name} 被 群${colors.magenta(this.groupList[event.group_id] || '')}(${colors.magenta(event.group_id.toString())}) - (${colors.magenta(event.operator_id.toString())}) 踢出群聊`, 'EVENT')
          }
        })
        .on('notice.group_increase', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          if (event.sub_type === 'approve') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.operator_id.toString())}) 同意 (${colors.white(event.user_id.toString())}) 加入群聊`, 'EVENT')
          } else if (event.sub_type === 'invite') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.operator_id.toString())}) 邀请 (${colors.white(event.user_id.toString())}) 加入群聊`, 'EVENT')
          }
        })
        .on('notice.group_ban', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          if (event.sub_type === 'ban') {
            PrintLog.logWarning(`群${colors.magenta(this.groupList[event.group_id] || '')}(${colors.magenta(event.group_id.toString())}) - (${colors.magenta(event.user_id.toString())}) 被 (${colors.magenta(event.operator_id.toString())}) 禁言 ${event.duration}秒`, 'EVENT')
          } else if (event.sub_type === 'lift_ban') {
            PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.user_id.toString())}) 被 (${colors.white(event.operator_id.toString())}) 解除禁言`, 'EVENT')
          }
        })
        .on('notice.group_upload', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - (${colors.white(event.user_id.toString())}) 上传了 ${colors.white(event.file.name)}`, 'EVENT')
        })
        ...
        .on('notice.group_card', async (event) => {
          if (Array.isArray(nolisten) && nolisten.includes(event.group_id)) return
          PrintLog.logNotice(`群${colors.white(this.groupList[event.group_id] || '')}(${colors.white(event.group_id.toString())}) - ${colors.white(event.card_old)}(${colors.white(event.user_id.toString())})群名片修改为${colors.white(event.card_new)}`, 'EVENT')
        })
